...
export default function LoginModal({ onClose, onLogin }: LoginModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-fadeIn" onClick={onClose}>
      <div 
        className="relative w-full max-w-4xl h-auto md:h-[700px] bg-slate-900/70 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-slideUp"
      ...
      >
        ...
        <div className="hidden md:flex w-full md:w-1/2 p-12 flex-col justify-between bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 relative overflow-hidden">
          <div className="absolute -top-16 -left-16 w-64 h-64 bg-white/10 rounded-full opacity-50 animate-pulse"></div>
          <div className="absolute -bottom-24 -right-10 w-72 h-72 bg-white/10 rounded-full opacity-50 animate-pulse animation-delay-2000"></div>
          <div>
            <h1 className="text-4xl font-bold text-white tracking-tight">RoomSplit</h1>
            <p className="text-white/80 mt-4 text-lg">Share expenses, not headaches. The simplest way to manage group finances.</p>
          </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative bg-slate-900">
          ...
          >
            <FiX className="w-6 h-6 transition-transform duration-300 group-hover:rotate-90" />
          </button>
          
          <div className="w-full max-w-sm mx-auto">
            <h2 className="text-3xl font-bold text-white mb-2">{isLogin ? 'Welcome Back' : 'Create Account'}</h2>
            <p className="text-slate-400 mb-8">{isLogin ? 'Enter your credentials to access your account.' : 'Join us and start splitting expenses.'}</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-2 px-6 py-4 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 text-white font-bold rounded-xl hover:opacity-90 focus:ring-4 focus:ring-blue-500/50 transition-all duration-300 transform hover:scale-[1.02] disabled:opacity-50 disabled:transform-none disabled:cursor-not-allowed group flex items-center justify-center gap-3"
              >
                {loading ? (
                  <>
